...
def bench_blob():
	...
	bench_blob_get_with_master_download_and_broadcast()
...
def bench_blob_get_with_master_download_and_broadcast():
	'''
	Benchmarking Blob get with master-read pattern
	...
	for _ in range(0, int(config_bench['repeat_time'])):
		...
		if RANK_MASTER == rank:
			...
			data = None
		data = comm.bcast(data, RANK_MASTER)
		end = MPI.Wtime()
		comm.Barrier()
	...
	if RANK_MASTER == rank:
		...
		print('-------- Single Blob, Master Broadcast --------')
		print('-------- Repeat {0} times --------'.format(config_bench['repeat_time']))
		print('-------- {0} KiB Inputs on {1} Processes --------'.format(round(blob_size/ 1024, 3), size))
		print('Max {0} s, Min {1} s, Avg {2} s'.format(max_read, min_read, avg_read))
